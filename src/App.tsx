...
function App() {
    ...
    const [shoplist, setShoplist] = useState<NewShopListType[]>([
        {
            title: "What to buy",
            filter: "All",
            goods: [
    ...
    const deleteGoods = (shoplistId: number, id: string) => {
        setShoplist(shoplist.map((el, index) => index === shoplistId ? {...el, goods: el.goods.filter(g => g.id !== id)} : el))
